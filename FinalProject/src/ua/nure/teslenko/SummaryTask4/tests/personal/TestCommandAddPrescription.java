...
public class TestCommandAddPrescription extends Mockito{
	...
	private static CommandAddPrescription addPrescription = new CommandAddPrescription();
	
	
	@Test
	public void executeNullLogin() {
		when(request.getParameter("patientId")).thenReturn(null);
		assertThrows(AppException.class, () -> addPrescription.execute(request, response));
	}
